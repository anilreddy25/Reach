...
@objc public protocol ScrollPagerDelegatefinal: NSObjectProtocol {
    @objc optional func scrollPagerfinal(scrollPager: ScrollPagerfour, changedIndex: Int)
}

@IBDesignable public class ScrollPagerfour: UIView, UIScrollViewDelegate{
    ...
    @IBOutlet public weak var delegate: ScrollPagerDelegatefinal!
    ...
    private func addButtons(titleOrImages: [AnyObject]) {
        ...
        for i in 0..<titleOrImages.count {
            ...
            button.tag = i
            button.addTarget(self, action: #selector(ScrollPagerfour.buttonSelected(sender:)), for: .touchUpInside)
    ...
    @objc internal func buttonSelected(sender: UIButton) {
        ...
        delegate?.scrollPagerfinal?(scrollPager: self, changedIndex: sender.tag)
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !animationInProgress {
            ...
            if Int(page) != selectedIndex {
                ...
                delegate?.scrollPagerfinal?(scrollPager: self, changedIndex: Int(page))
            }
        }
    }
