...
//  AgentRestocks
//
//  Created by Azharuddin on 30/11/18.
//  Copyright © 2018 NumberMall. All rights reserved.
//

import UIKit
import Alamofire
class ForgettwoViewController: UIViewController,UITextFieldDelegate
{
    ...
    @IBOutlet weak var MISSEDCALLNUMEBR: UILabel!
    
    
    @IBOutlet weak var missedcallbutton: UIView!
    
    
    @IBOutlet weak var bototmviewtwo: UIView!
    
    
    
    var MyMissedCalledNumbervaluefinal = String()
...
//        headerView.layer.shadowColor = UIColor.black.cgColor
        self.headerView.layer.addBorder(edge:.bottom, color:UIColor.textfiledbottomcolorside(), thickness:0.8)
        
        self.view.makeToast(messagefinalvalue)
        
        txtEnterOTP.delegate = self
        phonecall = "7799389551"
...
//
        ...
        if let MyMissedCalledNumbervaluefinaltring:String = defaults.value(forKey:"MyMissedCalledNumbervalue")as?String
        {
            self.MyMissedCalledNumbervaluefinal  = MyMissedCalledNumbervaluefinaltring
            self.MISSEDCALLNUMEBR.text =  self.MyMissedCalledNumbervaluefinal
        }
        ...
        self.MISSEDCALLNUMEBR.text = Missedcallfeaturetwo
    ...
    func Verifyotpmethod()
    {
        ...
        let postString =  String(format: "method=%@&mobilenumber=%@","verifyResetPinOTP",MobileStringfinalValue)
        // 680590
        print(postString)
    ...
    func VerifynumberMethod()
    {
        ...
        var request = URLRequest(url: URL(string:AgentConstants.baseUrltwo)!)
        request.httpMethod = "POST"
        let postString =  String(format: "method=%@&mobilenumber=%@&otp=%@&logintype=%@","verifyResetPinOTP",MobileStringfinalValue,"XXXXXX","4")
        request.httpBody = postString.data(using: .utf8)
        request.addValue("delta141forceSEAL8PARA9MARCOSBRAHMOS", forHTTPHeaderField: "Authorization")
        request.addValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
        request.addValue("application/json", forHTTPHeaderField: "Accept")
    ...
    func forgetpinmethod()
    {
        ...
        var request = URLRequest(url: URL(string:AgentConstants.baseUrltwo)!)
        request.httpMethod = "POST"
        let postString =  String(format: "method=%@&mobilenumber=%@&otp=%@&logintype=%@","resendOTPforforgotPin",Mobilenumbervalueforgetpassword,textenterfinalvaluefinal!,"4")
        print(postString)
        request.httpBody = postString.data(using: .utf8)
        request.addValue("delta141forceSEAL8PARA9MARCOSBRAHMOS", forHTTPHeaderField: "Authorization")
        request.addValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
                        ...
                        if  str2 == "1"
                        {
                            ...
                            DispatchQueue.main.async {
                                ...
                                let ResetpinViewController = self.storyboard?.instantiateViewController(withIdentifier:"ResetpinViewController")as!ResetpinViewController
                                    ResetpinViewController.messagevalue = msgvalue
