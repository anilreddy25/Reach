...
class contactsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    ...
    @IBOutlet weak var mynewgroupviewfinal: UIView!
    ...
    @IBOutlet weak var myinvitefriend: UIButton!
    ...
    override func viewDidLoad()
    {
       ...
       ContactList()
        ...
        self.mynewgroupviewfinal.layer.addBorder(edge:.bottom, color:UIColor.textfiledbottomcolorside(), thickness:0.8)
        self.inviteview.layer.addBorder(edge:.top, color:UIColor.textfiledbottomcolorside(), thickness:0.8)
        
        navigationbarview.layer.shadowOffset = CGSize(width: 0, height: 3)
        navigationbarview.layer.shadowOpacity = 0.6
        navigationbarview.layer.shadowRadius = 3.0
        navigationbarview.layer.shadowColor = UIColor.color2().cgColor
        
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.handleTapback(_:)))
        
        self.mynewgroupviewfinal.addGestureRecognizer(tap)
        
        self.mynewgroupviewfinal.isUserInteractionEnabled = true
    ...
    @objc func handleTapback(_ sender: UITapGestureRecognizer)
    {
        DispatchQueue.main.async
            {
            let GroupConatactViewFinalvalue = self.storyboard?.instantiateViewController(withIdentifier:"GroupConatactViewController")as!GroupConatactViewController
            self.present(GroupConatactViewFinalvalue, animated: true, completion:nil)
        }
    }
    func ContactList()
    {
