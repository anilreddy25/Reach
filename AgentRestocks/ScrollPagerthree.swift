...
@objc public protocol ScrollPagerDelegateThree: NSObjectProtocol {
    @objc optional func scrollPagersingelvalue(scrollPager: ScrollPagerthree, changedIndex: Int)
}

@IBDesignable public class ScrollPagerthree: UIView, UIScrollViewDelegate{
    ...
    @IBOutlet public weak var delegate: ScrollPagerDelegateThree!
    ...
    private func addButtons(titleOrImages: [AnyObject]) {
        ...
        for i in 0..<titleOrImages.count {
            ...
            button.tag = i
            button.addTarget(self, action: #selector(ScrollPagerthree.buttonSelected(sender:)), for: .touchUpInside)
    ...
    @objc internal func buttonSelected(sender: UIButton) {
        ...
        delegate?.scrollPagersingelvalue?(scrollPager: self, changedIndex: sender.tag)
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !animationInProgress {
                        ...
                        if Int(page) != selectedIndex {
                            ...
                            delegate?.scrollPagersingelvalue?(scrollPager: self, changedIndex: Int(page))
                        }
